...
def _journey_sections(data: dict[str, Any], *, csrf_token: str) -> str:
    ...
    communication_rows = ''.join(
        f'<tr><td>{_esc(str(row.get("milestone") or "update").replace("_", " ").title())}<small>Template v{_esc(row.get("template_version"))}</small></td><td>{_status(str(row.get("status") or "queued"))}</td><td>{_esc(_when(row.get("delivered_at") or row.get("sent_at")))}</td><td>{_esc(row.get("provider_reference") or row.get("last_error") or "No provider evidence")}</td><td>{f"<form method=\"post\" action=\"/admin/building/bookings/{_esc(reservation_id)}/communications/{_esc(row.get('milestone'))}/retry\"><input type=\"hidden\" name=\"_csrf_token\" value=\"{_esc(csrf_token)}\"><input type=\"hidden\" name=\"return_to\" value=\"{_esc(return_to)}#communications\"><button class=\"lead-button\" type=\"submit\">Retry</button></form>" if row.get("status") in {"failed", "not_configured"} else "—"}</td></tr>'
        for row in list(journey.get("communications") or [])
    ) or '<tr><td colspan="5">No later-stage customer messages yet.</td></tr>'
    ...
    for checklist in list(journey.get("checklists") or []):
        ...
        checklist_blocks += f'<div class="lead-journey-content"><h3>{_esc(checklist.get("title"))} · {_esc(str(checklist.get("status") or "open").replace("_", " ").title())}</h3><div class="lead-table"><table><thead><tr><th>Work</th><th>State</th><th>Owner</th><th>Due</th><th>Evidence</th><th>Action</th></tr></thead><tbody>{item_rows or "<tr><td colspan=\"6\">No checklist items.</td></tr>"}</tbody></table></div></div>'
