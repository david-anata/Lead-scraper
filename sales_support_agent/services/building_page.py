...
def render_building_page(
    *,
    ...
    launch_definitions = [
        ...
        ("tax_treatment", "Tax treatment and rate", "accepted_policy", "Taxable/non-taxable and reviewed rate", "David decided 2026-07-31 that Arena rental is not taxable. This is an owner decision, not an accountant determination, so it carries the risk of uncollected tax if Utah disagrees. Discounted quotes stay blocked until it is saved on the rate plan.", "Save tax_status=non_taxable on the Arena rate plan, with David's decision as the evidence."),
        ...
        ("agreement_template", "Reusable agreement template", "approved_reference", "Approved provider-neutral template ID/version", "Google Docs eSignature is the approved signing workspace; Agent creates the signing copy and records provider evidence but does not claim delivery or signature without that evidence. The 2025 Vivint agreement is customer-specific and is not reusable.", "Approve a reusable legal template and a legal approver; the e-sign provider is decided."),
...
<head>
  ...
  <style>
    ...
    .decision-card__evidence p{{margin:5px 0 0;color:rgba(43,54,68,.7);font-size:12px;line-height:1.5;}}
    ...
    <div class="grid">
      ...
      <section class="panel panel--wide" id="commercial-rate-plans">
        ...
        <form class="form-grid" method="post" action="/admin/building/rate-plans">
          ...
          <details class="field field--wide advanced-disclosure">
            ...
            <div class="form-grid">
              <div class="field"><label for="rate-version">Version</label><input id="rate-version" name="version" type="number" min="1" value="1"><small class="field-help">Counts up each time you replace a price list.</small></div>
              <div class="field"><label for="rate-id">Reference</label><input id="rate-id" name="rate_plan_id" placeholder="Left blank, we name it for you"><small class="field-help">Built from the offering and version, like arena-events-v1.</small></div>
              <div class="field"><label for="rate-status">Save as</label><select id="rate-status" name="status"><option value="draft">Draft</option><option value="in_review">Ready for review</option></select></div>
      ...
      <section class="panel panel--wide building-view view-sales" id="add-assisted-lead">
        ...
        <form class="form-grid" method="post" action="/admin/building/inquiries">
          ...
          <input type="hidden" name="consent_to_contact" value="true">
