    ...
    if content_strategy:
        lines.extend(
            [
                ...
                f"- Daily article minimum: `{content_strategy.get('daily_article_minimum', 8)}`",
                f"- Daily article target: `{content_strategy.get('daily_article_target', 8)}`",
...
<body>
  <main>
    ...
    <div class="panel">
      ...
      <p><strong>Editorial program:</strong> {esc(content_strategy.get('total_briefs', 0))} briefs · {esc(content_strategy.get('ready_to_publish', 0))} ready · {esc(content_strategy.get('researching_sources', 0))} researching sources · daily minimum {esc(content_strategy.get('daily_article_minimum', 8))} · target {esc(content_strategy.get('daily_article_target', 8))}</p>
