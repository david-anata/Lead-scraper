...
def _smart_cfo_section_html(analysis: Mapping[str, Any]) -> tuple[str, dict[str, Any]]:
    ...
    if status == "not_configured":
        ...
        content = f'''<div class="finance-savings-state"><strong>Smart review found no eligible actions.</strong><p>{html.escape(str(analysis.get("summary") or "The current ledger did not support a recommendation."))}</p><span>Current operating ledger · {html.escape(str(analysis.get("record_count") or 0))} records</span><form method="post" action="/admin/finances/smart-review"><button class="btn btn-secondary btn-sm" type="submit">Re-run on current ledger</button></form></div>'''
    elif not recommendations:
        content = '''<div class="finance-savings-state"><strong>Run a Smart review when you want a CFO read.</strong><p>It evaluates current operating activity and all unresolved obligations, never a sample, and never changes cash or source records.</p><form method="post" action="/admin/finances/smart-review"><button class="btn btn-primary btn-sm" type="submit">Run Smart review</button></form></div>'''
    else:
        rows = []
