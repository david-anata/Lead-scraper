...
| Finance | Finance Today and read APIs render from Supabase; Finance categories remain distinct | Invalid QuickBooks state and unsigned Plaid webhook reject; runtime role cannot execute DDL | Plaid, QuickBooks, reconciliation, evidence, preview/apply/undo, permissions, and renderer tests pass | Register callbacks and run sandbox/read-only QuickBooks and Plaid flows; no money movement is authorized |
...
failure boundaries are verified on staging. Provider-console registration and
controlled source-system receipts remain the only integration-parity work that
