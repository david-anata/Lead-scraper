...
from sales_support_agent.config import ACTIVE_FOLLOW_UP_STATUSES, DEFAULT_STATUS_POLICIES
...
class FollowUpRuleTests(unittest.TestCase):
    def test_follow_up_status_is_not_in_automated_scan_statuses(self) -> None:
        self.assertNotIn("FOLLOW UP", ACTIVE_FOLLOW_UP_STATUSES)
    ...
    def test_follow_up_status_policy_still_exists_for_manual_use(self) -> None:
        assessment = assess_status_follow_up(
            status="FOLLOW UP",
            policy=DEFAULT_STATUS_POLICIES["FOLLOW UP"],
            created_at=datetime(2026, 3, 13, 9, 0, 0),
